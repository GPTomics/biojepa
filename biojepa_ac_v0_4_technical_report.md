...
This report presents BioJEPA-AC (Biological Joint-Embedding Predictive Architecture - Action Conditioned), a joint-embedding predictive architecture for modeling perturbation-conditioned transcriptional responses in pooled single-cell perturbation screens. 
...
### 1.1 The Challenge of Perturbation Response Modeling

Large-scale single-cell perturbation screens (e.g., CRISPRi Perturb-seq) produce paired information containing a perturbation identity and a high-dimensional transcriptional readout. The K562 Perturb-seq resource associated with Replogle et al. provides a reference setting for this problem^[1]^. With BioJEPA-AC, the modeling goal is to build a model capable of learning both cell states and perturbation impacts while starting from a control-state distribution to a perturbed-state distribution, with explicit conditioning on perturbation metadata.

### 1.2 Why Predicting Perturbation Responses is Hard

Several properties of scRNA-seq perturbation screens motivate learning in a latent space rather than directly optimizing expression-space reconstruction:
...
To target some of the shortcomings of existing approaches, BioJEPA-AC adopts the Joint-Embedding Predictive Architecture (JEPA) paradigm, pioneered in computer vision and grounded in the energy-based model theory of LeCun.  The fundamental insight of JEPA is to abandon the reconstruction of raw inputs entirely. Instead, the model learns to predict the latent representation of the missing information based on the latent representation of the context.^[2]^ Related recent perturbation-response methods include distributional predictors such as PerturbNet^[3]^ and embedding-conditioned predictors such as Scouter^[4]^

In this framework, the loss function operates in the abstract embedding space, not the input space. This allows the model to become invariant to irrelevant details (such as technical sequencing noise or batch effects) while remaining sensitive to semantic information (such as cell type, cell cycle phase, or perturbation response). The objective shifts from "What are the exact counts of the masked genes?" to "What is the biological state represented by the masked genes?".

### 1.5 BioJEPA-AC: The Action-Conditioned World Model

BioJEPA-AC can be interpreted through a JEPA/energy-based learning lens in which training minimizes a discrepancy between predicted and target latents without specifying an explicit likelihood in expression space^[2]^. This perspective is compatible with action-conditioned JEPA formulations in other domains (e.g. V-JEPA 2)^[5]^. In BioJEPA-AC, the predictor is conditioned on an action embedding and is trained to assign low discrepancy to teacher latents consistent with the observed perturbed cell and the provided perturbation metadata.
...
The manifold hypothesis suggests that while scRNA-seq data exists in a high-dimensional Euclidean space ($\mathbb{R}^{20000}$), the biologically valid states lie on a much lower-dimensional manifold. Generative models often struggle because they attempt to approximate the probability density of the entire high-dimensional space. BioJEPA-AC, by projecting data into a low-dimensional latent bottleneck (e.g., $\mathbb{R}^{256}$ in implementation), implicitly learns the topology of this manifold.

The "Action-Conditioned" component essentially learns vector fields on this manifold. If the manifold represents the landscape of possible cellular identities (Waddington's epigenetic landscape), the action embeddings represent the forces that push cells along specific trajectories (valleys) within that landscape.
...
BioJEPA-AC attempts to internalize this causality through its prediction task. By forcing the model to predict the consequence of an action $a$ on the context $z$, the model must learn the directional edges of the gene regulatory network (GRN). The action-conditioned formulation treats a perturbation label (and its metadata) as an intervention variable \(a\) that conditions the mapping from control to perturbed states^[2,5]^. *This moves the field from descriptive "atlasing" to predictive "simulation".*
...
- **Cell State Encoder:** maps gene expression to contextualized latent gene tokens. This is used for both the student and teacher
- **Action Composer:** maps perturbation metadata to a fixed-dimensional action latent.
- **Action-Conditioned Predictor:** predicts perturbed latent tokens conditioned on control latent tokens and the action latent.

### 3.1 Linear Projection with Gene Identity

Gene identity is represented via a learned embedding, and expression magnitude is incorporated via an explicit value encoding (including a total-count feature) before passing tokens through transformer layers .

- **Gene Embeddings:** Each gene $i$ has a learned embedding vector $e_i \in \mathbb{R}^d$.
- **Expression Scaling:** The scalar expression value $x_i$ scales the embedding: $h_i = x_i \cdot e_i$
- **Total Count Bias Injection:** The total mRNA count (library size) is projected to a vector and added as a bias term to every gene embedding. This explicitly models total detected genes as a global covariate allowing models to learn when cells are no longer viable.  

This "bag-of-genes" approach preserves the continuous nature of the data and allows the model to differentiate between genes solely based on their learned identity embedding $e_i$.

### 3.2 Cell State Encoder

The cell-state encoder is a transformer encoder operating over gene tokens and uses a feature-map (linear) attention variant to reduce quadratic scaling^[16]^. The encoder produces contextualized latent tokens \(z \in \mathbb{R}^{G \times D}\) .

#### 3.2.1 Linear Attention Mechanism

A major bottleneck in applying transformers to scRNA-seq is the quadratic complexity of self-attention ($O(L^2)$), where $L$ is the number of genes. Since a cell can express >5,000 genes, standard attention is computationally prohibitive. The implementation utilizes a shallow set transformer with an efficient linear attention mechanism.

- **Structure:** $N=6$ layers, $H=4$ heads, Embedding Dimension $d=256$.
- **Mechanism:** Instead of softmax attention, the encoder uses a linear attention variant based on a non-negative feature map (e.g., $\phi(x)=\mathrm{ELU}(x)+1$), reducing attention complexity to $O(N \cdot L)$ (linear in the number of genes).^[16]^
- **Permutation Invariance:** The encoder treats genes as an unordered set (no positional encoding), aligning with the biological reality that the physical order of genes on a chromosome does not strictly dictate their regulatory function in trans.

#### 3.2.2 The Student-Teacher Framework

BioJEPA-AC maintains a student encoder and , to provide stable targets for the predictor, an exponential moving average (EMA) teacher encoder that is structurally identical to the student encoder. 
...
$$
In practice, this means teacher processes the target view ($x_{target}$) to generate the ground-truth embedding $z_{target}$, preventing "target chasing" instabilities.

#### 3.2.3 Masking Strategy

Training randomly masks a subset of gene positions and predicts their teacher latents. We use a default mask ratio is 0.6 (60%), which is consistent with masked-reconstruction regimes used in other domains (e.g., masked autoencoders)^[15]^. The mask-and-predict workflow is used both for pretraining and for action-conditioned training ^[14,15]^.

### 3.3 The Action Composer

The defining feature of the "AC" variant is the capability to encode heterogeneous perturbations into a unified latent space. The Action Composer handles this unification.
...
These features are projected via linear layers into a shared content latent space $c \in \mathbb{R}^{320}$.

Since we limited our current training to the K562 Essential dataset, our only perturbation at this point is CRISPRi^[1]^.  To process these perturbation we created  embeddings for the sgRNA using the DNA modality above and converted the target gene identifiers to protein sequences which we embedded with ESM-2.
...
The action predictor network functions as the causal simulator. It consumes control latents \(z_c\) and action latent \(a\), forms learnable target queries for masked genes, and applies attention blocks to produce per-gene predicted latents

- **Cross-Attention Injection:** The predictor is a Transformer decoder. The action latent $a$ is treated as the **Key (K)** and **Value (V)**, while the cell state tokens act as **Queries (Q)**. This "injects" the perturbation information globally into every gene token.
- **Dynamics Propagation:** Subsequent self-attention layers allow the gene tokens to update their states based on the injected perturbation and their neighbors, modeling the propagation of the signal through the gene regulatory network.
- **Stochastic Output:** The predictor outputs two heads for each gene latent: a mean $\mu$ and a log-variance $\log \sigma^2$. This allows the model to estimate epistemic uncertainty, identifying genes where the prediction is low-confidence (e.g., due to sparsity or lack of training data).
...
$$
with VICReg regularization to discourage collapse^[6]^. The pretraining updates both the student and teacher.^[14]^,^[15]^. The objective is to learn robust gene embeddings and cellular representations without perturbation labels.

- **Masking Strategy:** For a given cell, 60% of the gene tokens are masked.
- **Architecture:** The Student encoder processes the masked input, while the Teacher encoder processes the full input.
- **VICReg Loss:** BioJEPA-AC eschews contrastive losses in favor of VICReg (Variance-Invariance-Covariance Regularization) to prevent mode collapse.
- **Teacher Update:** The teacher's weights are updated from the student via exponential moving average based on a specified momentum set to 0.996.

#### 5.1.1 The VICReg Loss Function

The loss $\mathcal{L}$ is a weighted sum of three loss terms. 
\[
...
After pretraining and embedding training, the ACpredictor is trained to map control latents and action embeddings to perturbed latents using masked NLL:
$$
...
The full model is trained on paired (Control, Perturbation, Case) data. The Student receives the Control Cell state + Perturbation Action Vector. The Teacher receives the Perturbed Cell state. Crucially, the student and teacher encoders are frozen during this phase. This treats the encoders as a fixed feature extractor, preserving the general manifold structure learned during pretraining and preventing overfitting to the smaller perturbation dataset. A combination of reconstruction loss (NLL on masked gene latents) and VICReg consistency loss on the global representation.
\[
