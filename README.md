...
11. **When appropriate remove unknown positions from evals:** Added unknown masking to several evals that look at gene position values against the input gene values to ensure that we only compare the genes where we have a source of truth. 
     1. Benefit: Previously we treated unknown genes as 0 so we were penalizing our benchmark for any learning of coexpression. By removing the unknown genes from our evals, we mirror the change in our loss function and ensure that our model is only focusing on benchmarking the genes it knows. 
