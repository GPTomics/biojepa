...
8. **SigLIP Sigmoid Loss for Composer Training:** Added an optional SigLIP (Sigmoid Loss for Language-Image Pre-training) loss as an alternative to InfoNCE cross-entropy for alignment training. Controlled by `loss_type` config field (`'infonce'` default, `'siglip'`). SigLIP replaces the softmax-normalized cross-entropy with independent pairwise sigmoid losses: each (seq, target) pair contributes gradients independently, removing the softmax bottleneck where only 63 negatives exist at batch_size=64. A learnable bias parameter (`siglip_bias`, initialized at -10.0 per the SigLIP paper) sets the decision boundary. Temperature remains fixed at 0.012 from HPO.
    1. Benefit: At small batch sizes, InfoNCE's softmax normalization dilutes gradient signal across few negatives. SigLIP treats each of the B^2 pairs independently, providing stronger per-pair gradients. The SigLIP paper shows sigmoid loss significantly outperforms softmax InfoNCE at batch sizes below 16K. The learned bias naturally handles the positive/negative imbalance (64 positives vs 4,032 negatives at BS=64).
