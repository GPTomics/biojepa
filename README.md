...
3. **Full-Position AC Loss + Mask Annealing to Zero:** NLL loss is now computed on all gene positions rather than only masked positions. Added `mask_anneal_floor` config field (default 0.0) replacing the hardcoded floor of 0.1, enabling clean annealing all the way to mask_ratio=0.0.
...
7. **InfoNCE Loss for Composer Training:** Composer alignment uses seq-to-target InfoNCE: cross-entropy over a temperature-scaled cosine similarity matrix. Temperature is a fixed hyperparameter (default 0.012 from HPO). Asymmetric (seq-to-target only) because the structural asymmetry between pathways (1536d sequences vs 320d targets) makes symmetric loss harmful.
    1. Benefit: Softmax over the full batch forces every positive pair to rank above all negatives simultaneously, producing highly discriminative embeddings with high cosine similarity. With ~10.8K training pairs and batch_size=64, InfoNCE extracts maximum learning signal per batch.
8. **Dose Conditioning for AC Training:** Added a `dose_proj` MLP to ActionComposer that applies multiplicative dose scaling to each perturbation slot's action latent after mode conditioning. The dataloader applies `log1p` to raw dose values (compressing the 1000x range of 10-10000 nM to ~2.4-9.2) while preserving the -1.0 sentinel for genetic perturbations. Identity initialization (zero weights, ones bias on the final layer) ensures existing checkpoint compatibility via `strict=False` loading. The `dose=None` default on all modified signatures means non-dose paths have zero overhead.
...
9. **Removed Attention Pooling from ActionComposer:** Removed the learned `pool_query` parameter and `nn.MultiheadAttention` pooling layer. Composer training always uses single-pert format (N_pert=1), so the attention pool was functioning as a learned linear transform rather than actual pooling. Replaced with `.squeeze(1)` in `forward_composer`. Multi-pert eval (`_multi_pert_alignment`) uses a simple mean pool.
    1. Benefit: Removes unnecessary parameters and complexity. The pool layer's weights were trained exclusively on single-token input, so they never learned meaningful pooling behavior. Also removes the head-divisibility constraint on `pert_latent_dim` that existed solely because of `nn.MultiheadAttention`.
10. **Additive Gene Identity Baseline:** Added `gene_embeddings` as an unconditional additive term to the CellStateEncoder input encoding, applied after FiLM modulation and before mask/unknown token replacement. The encoding is now `x = gene_emb + (gene_emb * expr_scale * (1 + gamma) + beta)`, ensuring gene identity is always present for measured genes regardless of expression level. Mask token and unknown token still fully overwrite at their respective positions.
    1. Benefit: Previously, genes with zero expression (typically ~50% of measured positions per cell) lost all gene identity through the multiplicative coupling (`gene_emb * 0 = 0`), producing identical representations for thousands of distinct genes. Since linear attention is equivariant over identical inputs, this identity loss was irrecoverable through all transformer layers. The additive baseline preserves gene identity at zero expression, matching how every major single-cell foundation model (scGPT, UCE, scFoundation) handles gene/positional encoding and reflecting the biological reality that a gene's identity exists independently of its current expression level. VICReg coefficients from v0.6 HPO (std_coeff=40.5) were likely inflated to compensate for the artificial variance collapse caused by ~50% identical representations, and may need re-tuning.
11. **When appropriate remove unknown positions from evals:** Added unknown masking to several evals that look at gene position values against the input gene values to ensure that we only compare the genes where we have a source of truth. The `_RunningMeans` class tracks per-gene counts so that when the same perturbation appears across datasets with different gene coverage, each gene's mean is computed only from samples that actually measured it. Per-dataset and per-cell-type gene masks are derived from their own `_RunningMeans` instances rather than a global first-seen lookup.
     1. Benefit: Previously we treated unknown genes as 0 so we were penalizing our benchmark for any learning of coexpression. By removing the unknown genes from our evals, we mirror the change in our loss function and ensure that our model is only focusing on benchmarking the genes it knows. The per-gene averaging prevents cross-dataset dilution where unmeasured positions (zero-filled) corrupted running means for the ~285 perturbations shared between k562e_raw and k562gw.
12. **Teacher Encoder for All Downstream Use:** AC training, decoder training, and all evaluations now use the teacher (EMA/target) encoder exclusively. Previously, the student (context) encoder was used for AC context encoding, decoder training, and all evals, while the teacher encoded AC targets. This mixed two representation spaces. Now the student encoder is only used during encoder pretraining (receives gradients, feeds teacher via EMA). The teacher is the deployed model.
    1. Benefit: Aligns with the JEPA family convention. V-JEPA 2-AC, DINO, I-JEPA, and V-JEPA all use the teacher encoder for inference and downstream tasks. EMA-smoothed representations are more stable and often higher quality than the gradient-trained student. Using the teacher for both AC context and target eliminates the representation space mismatch where the predictor learned teacher-space dynamics but the decoder and evals operated in student-space.
